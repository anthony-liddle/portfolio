...
The second piece was the voting record. Most civic tools stop at contact info. I wanted a user writing to their rep about climate policy to immediately see how that rep has voted on climate-related bills. That required building Voteprint: a custom donut-ring chart where wedges correspond to issue categories, spokes correspond to individual roll call votes, and spoke direction and length encode whether the rep's vote aligned with or opposed a leftist position on the issue.
...
It's a tradeoff, not a clean answer, and I want the case study to be honest about that
